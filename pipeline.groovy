pipeline {
    ...
    environment {
        ...
        AUTO_PREPROD_API_KEY = credentials('jenkins-pdpl-apikey')
        AUTO_PROD_API_KEY = credentials('jenkins-pdpl-apikey')
    ...
    stages {
        stage('PREPARE'){
            steps {
                cleanWs()
                sh 'echo ${bundle_name}'
                git credentialsId: "git_hub_ssh", url: "git@github.com:roolrd/dss_pipeline_pdpl-master.git"
